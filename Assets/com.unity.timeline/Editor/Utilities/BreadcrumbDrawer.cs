...
namespace UnityEditor.Timeline
{
    ...
    class BreadcrumbDrawer
    {
        ...
        static readonly GUIStyle k_BreadCrumbLeft = "GUIEditor.BreadcrumbLeft";
        static readonly GUIStyle k_BreadCrumbMid = "GUIEditor.BreadcrumbMid";
                ...
                for (var i = 0; i < labels.Count; i++)
                {
                    ...
                    if (i == labels.Count - 1)
                    {
                        if (i > 0)
                        {
                            ...
                            DrawBreadcrumbAsSelectedSubSequence(labelWidth, label, k_BreadCrumbMidSelected);
                        }
                        else
                        {
                            DrawActiveBreadcrumb(labelWidth, label, style);
                        }
                    }
                    else
                    {
                        ...
                        if (GUI.Button(GetBreadcrumbLayoutRect(content, style), content, style))
                        {
        ...
        static void DrawBreadcrumbAsSelectedSubSequence(int width, BreadCrumbTitle label, GUIStyle style)
        {
            var rect = DrawActiveBreadcrumb(width, label, style);
            const float underlineThickness = 2.0f;
            const float underlineVerticalOffset = 1.0f;
            var underlineHorizontalOffset = style.border.right * 0.333f;
        ...
        static Rect DrawActiveBreadcrumb(int width, BreadCrumbTitle label, GUIStyle style)
        {
